...
namespace badgerdb
{
	...
	// const  int INTARRAYLEAFSIZE = 1200;
	 ...
	 */
	 ...
	 //const  int INTARRAYNONLEAFSIZE = 2000;
	/**
	...
	*/
	...
	private:
	   ...
	   */
		...
		/**
		 * Record if the original roof was spilt
		 */
		int if_rootleaf;

		void tree_search(PageId PageNum);

		void ChooseSubtree(NonLeafNodeInt* Node, PageId& nextpageNum, int key);

		void insert(Page* curPage, PageId curPageNum, bool isLeaf, const RIDKeyPair<int> entry, PageKeyPair<int>*& newchildentry);

		void insert_Leaf(LeafNodeInt* leaf, RIDKeyPair<int> entry);

		void insert_nonLeaf(NonLeafNodeInt* nonleaf, PageKeyPair<int>* entry);

		void split_nonLeaf(NonLeafNodeInt* oldNode, PageId oldPageNum, PageKeyPair<int>*& newchildEntry);

		void new_root(PageId firstPageInRoot, PageKeyPair<int>* newchildEntry);

		void split_Leaf(LeafNodeInt* leaf, PageId leafPageNum, PageKeyPair<int>*& newchildentry, const RIDKeyPair<int> entry);
