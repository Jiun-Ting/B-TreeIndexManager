...
const int	relationSize_L = (INTARRAYNONLEAFSIZE+1+1) * (INTARRAYLEAFSIZE/2);
const int	relationSize_L2 = relationSize_L+1;
...
void createRelationForward(const int = relationSize);
void createRelationBackward(const int = relationSize);
void createRelationRandom(const int = relationSize);
void createRelationForward_neg(const int = relationSize);

void intTests();
void intTests_large();
void intTests_empty();
void intTests_negative();
void intTests_outrange();

int intScan(BTreeIndex *index, int lowVal, Operator lowOp, int highVal, Operator highOp);
void indexTests();
void indexTests_large();
void indexTests_empty();
void indexTests_negative();
void indexTests_outrange();
...
void test_outrange();
void test_negative();
void test_LargeRelation(const int);
void print_tree();
...
int main(int argc, char **argv)
{
  ...
  std::cout << "page size:" << Page::SIZE << "PageId size: " << sizeof(PageId) << "int size: "<< sizeof(int)  << "RecordId size: " << sizeof(RecordId) << std::endl;
  ...
  try
	...
	test_negative();
	//print_tree();
	// The following two test add "wide range scan" check and "full range" scan check

		// relationSize = 1024*341+341: Split once and more entry within a leaf
	 test_LargeRelation(relationSize_L);

		// relationSize = 1024*341+341: Split once and more entry over a leaf
 	test_LargeRelation(relationSize_L2);

	// add NoSuchKeyFoundException check
    //     IndexScanCompletedException check
	errorTests();
...
void test_LargeRelation(const int Size)
{
	// Create a relation with tuples valued 0 to relationSize in random order and perform index tests 
	// on attributes of all three types (int, double, string)
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationForeward (Size: "<< Size << ")" << std::endl;
	createRelationForward(Size);
	indexTests_large();
	deleteRelation();

	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationBackward (Size: " << Size << ")" << std::endl;
	createRelationBackward(Size);
	indexTests_large();
	deleteRelation();

	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationRandom (Size: " << Size << ")" << std::endl;
...
void test_empty()
{
	...
	// on attributes of all three types (int, double, string)
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationForeward (Size: 0)" << std::endl;
	createRelationForward(0);
	indexTests_empty();
	deleteRelation();

	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationBackward (Size: 0)" << std::endl;
	createRelationBackward(0);
	indexTests_empty();
	deleteRelation();

	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationRandom (Size: 0)" << std::endl;
...
void test_outrange()
{
	...
	// on attributes of all three types (int, double, string)
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationForeward" << std::endl;
	createRelationForward();
	indexTests_outrange();
	deleteRelation();
...
void createRelationForward(const int size)
{
...
void createRelationForward_neg(const int size)
{
	std::vector<RecordId> ridVec;
	// destroy any old copies of relation file
	try
	...
	for (int i = 0; i < size; i++)
	{
		sprintf(record1.s, "%05d string record", i - relationSize + 1);
		record1.i = i - relationSize + 1;
		record1.d = (double)i - relationSize + 1;
		std::string new_data(reinterpret_cast<char*>(&record1), sizeof(record1));
...
void createRelationBackward(const int size)
{
  // destroy any old copies of relation file
	try
	{
...
void createRelationRandom(const int size)
{
  // destroy any old copies of relation file
	try
	{
...
void intTests()
{
  ...
  std::cout << "Created a B+ Tree index on the integer field" << std::endl;
	...
	checkPassFail(intScan(&index, 0, GTE, 10, LTE), 11)
	checkPassFail(intScan(&index, 4990, GTE, 4999, LTE), 10)
	checkPassFail(intScan(&index, 0, GTE, 0, LTE), 1)
	checkPassFail(intScan(&index, 5000, GTE, 5000, LTE), 0)
}
...
void intTests_outrange()
{
	...
	// run some tests
		...
		checkPassFail(intScan(&index, 100, GTE, 4999, LTE), 4900)
		checkPassFail(intScan(&index, -100, GT, 5100, LT), 5000)
}
...
void intTests_large()
{
	...
	// run some tests
		checkPassFail(intScan(&index, 0, GTE, 40, LT), 40)
		...
		//Wide Range
		checkPassFail(intScan(&index, 33000, GTE, 73000, LT), 40000)

		//Full Range
		checkPassFail(intScan(&index, 0, GTE, relationSize_L, LT), relationSize_L)
}
...
void intTests_negative()
{
	...
	// run some tests
	checkPassFail(intScan(&index, -25, GT, -4, LT), 20)
	checkPassFail(intScan(&index, -20, GTE, -3, LTE), 18)
	checkPassFail(intScan(&index, -996, GT, -100, LT), 895)
	checkPassFail(intScan(&index, -300, GT, -4, LT), 295)
	checkPassFail(intScan(&index, -3000, GTE, -40, LT), 2960)
...
int intScan(BTreeIndex * index, int lowVal, Operator lowOp, int highVal, Operator highOp)
{
  ...
  int numResults = 0;
    ...
    std::cout << "startScan Fail: No Key Found satisfying the scan criteria." << std::endl;
		return 0;
	}
...
void errorTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	int int10000 = 10000;
	int int100000 = 100000;
	int int10_m = -10;
	int int100_m = -1000;

	std::cout << "Scan with NoSuchKeyFoundException (input values both larger than the maximun of index)" << std::endl;
	try
	{
		index.startScan(&int10000, GTE, &int100000, LTE);
		std::cout << "NoSuchKeyFoundException Test 1 Failed." << std::endl;
	}
	catch (NoSuchKeyFoundException e)
	{
		std::cout << "NoSuchKeyFoundException Test 1 Passed." << std::endl;
	}

	std::cout << "Scan with NoSuchKeyFoundException (input value both smaller than the minimun of index)" << std::endl;
	try
	{
		index.startScan(&int100_m, GTE, &int10_m, LTE);
		std::cout << "NoSuchKeyFoundException Test 2 Failed." << std::endl;
	}
	catch (NoSuchKeyFoundException e)
	{
		std::cout << "NoSuchKeyFoundException Test 2 Passed." << std::endl;
	}

	std::cout << "Scan too many times" << std::endl;
	RecordId scanRid;
	...
	try
	{
		...
		std::cout << "IndexScanCompletedException Test 2 Failed." << std::endl;
	}
	catch (IndexScanCompletedException e)
	{
		std::cout << "IndexScanCompletedException Test 2 Passed." << std::endl;
	}
...
void print_tree() {
	...
	while (1)
	{
		try {
			index.scanNext(scanRid);
			std::cout << "page: " << scanRid.page_number << std::endl;
			std::cout << "slot: " << scanRid.slot_number << std::endl;
